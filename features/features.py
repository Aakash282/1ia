...
def feature_set(year):
    ...
    header = ['away_' + x for x in temp_features['away']] + \
             ['home_' + x for x in temp_features['away']] + ['score diff', 'spread', 'roof', 'timeofgame', 'week_year']
    ...
    added_columns = [('3rd_down_conv_p', '3rd_down_converted', '3rd_down_attempts'), \
                         ('4th_down_conv_p', '4th_down_converted', '4th_down_attempts'),
                         ...
                         ('pass_comp_p', 'pass_comp', 'pass_attempt'),
                         ('rush_yards_p', 'rush_yards', 'total_yards'),
                         # PassTD/Int might be too noisy?
                         ('PassTD_div_INT', 'pass_TDs', 'INT')]
...
def divide_features(table, feature_name, feature_numerator, feature_divisor):
    ...
    for elem in ['away_', 'home_', 'away_opp_', 'home_opp_']:
